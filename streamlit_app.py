...
# --------- تحديث دالة تمييز الكلمات لتعمل مع النص المُنَظَّم (بدون مدود) ---------
def highlight_keywords(text, keywords, normalized_keywords=None, exact_match=False):
    ...
    # نحتاج لإزالة المدود من النص الأصلي ومن الكلمات المفتاحية
    normalized_text = normalize_arabic_text(text)
    normalized_keywords = [normalize_arabic_text(kw) for kw in keywords] if keywords else []
    marked_spans = []

    # أولاً: المطابقات التامة
    for i, kw in enumerate(keywords):
        norm_kw = normalized_keywords[i]
        if not kw or not norm_kw:
            continue
        # ابحث عن تطابق تام للكلمة بعد التطبيع
        for m in re.finditer(r'(?<!\w)' + re.escape(norm_kw) + r'(?!\w)', normalized_text, re.IGNORECASE):
            # احسب موضع المطابقة في النص الأصلي بناءً على طول الكلمة
            # حاول مطابقة substring في النص الأصلي مع الموضع في النص المنظم
            start_norm = m.start()
            end_norm = m.end()
            # ابحث عن substring مطابق في النص الأصلي بنفس الطول
            orig_start, orig_end = find_match_in_original(text, normalized_text, start_norm, end_norm)
            marked_spans.append((orig_start, orig_end, "exact"))
    ...
    if normalized_keywords:
        for i, norm_kw in enumerate(normalized_keywords):
            if not norm_kw:
                continue
            # ابحث عن جميع مواقع المطابقات الجزئية:
            for m in re.finditer(re.escape(norm_kw), normalized_text, re.IGNORECASE):
                overlap = False
                start_norm = m.start()
                end_norm = m.end()
                orig_start, orig_end = find_match_in_original(text, normalized_text, start_norm, end_norm)
                for s, e, t in marked_spans:
                    if not (orig_end <= s or orig_start >= e):
                        overlap = True
                        break
                if not overlap:
                    marked_spans.append((orig_start, orig_end, "partial"))
...
def find_match_in_original(original, normalized, start_norm, end_norm):
    """
    ابحث عن substring في النص الأصلي الذي يتطابق مع substring في النص المنظم بين start_norm و end_norm.
    تفترض أن الطول قد يختلف بسبب إزالة المدود، لذا تبحث عن أول substring في النص الأصلي يطابق النص المنظم المطلوب.
    """
    norm_sub = normalized[start_norm:end_norm]
    # استخدم نافذة منزلقة للبحث في النص الأصلي
    max_len = end_norm - start_norm + 4  # هامش صغير
    for i in range(max(0, start_norm-2), min(len(original), start_norm+max_len)):
        for j in range(i+1, min(len(original), i+max_len+2)):
            sub = original[i:j]
            if normalize_arabic_text(sub) == norm_sub:
                return i, j
    # إذا لم يتم العثور: استخدم تقريبًا موضع البداية والنهاية
...
def normalize_arabic_text(text):
    # إزالة جميع المدود "ـ" وأي تكرار متتالي لأي حرف (مد طويل)
    text = re.sub(r'[ـ]', '', text)
    text = re.sub(r'(.)\1{2,}', r'\1', text)  # أي حرف متكرر أكثر من مرتين يصبح مرة واحدة
