...
# --------- تم تعديل هذه الدالة لتمييز المطابقة الجزئية ---------
def highlight_keywords(text, keywords, normalized_keywords=None, exact_match=False):
    ...
    if normalized_keywords:
        ...
        for i, norm_kw in enumerate(normalized_keywords):
            ...
            # ابحث عن وجود الكلمة بشكل جزئي (وليس تطابق تام)
            # أي: الكلمة ضمن كلمة أخرى أو كجزء من كلمة
            # لا تميّز المطابقات التامة (المؤشر عليها بالفعل)
            # (نطبّق على النص الأصلي حتى لا تتأثر العلامات السابقة)
            # هنا نستخدم تطابق جزئي فقط إذا exact_match = False
            if not exact_match:
                # ابحث عن جميع التطابقات الجزئية التي ليست بالفعل مظللة بـ<mark>
                # اجلب كل التطابقات الجزئية في النص الأصلي
                part_pattern = re.compile(re.escape(original_kw), re.IGNORECASE)
                def part_replacer(m):
                    # إذا كانت هذه المطابقة ليست ضمن <mark> ... </mark>
                    span_start = m.start()
                    span_end = m.end()
                    # تأكد أن ما حول المطابقة ليس ضمن علامة <mark>
                    before = text[max(0, span_start-6):span_start]
                    after = text[span_end:span_end+7]
                    if "<mark" in before and "</mark>" in after:
                        return m.group(0)  # لا تكرر التمييز
                    return f'<mark class="mark-soft">{m.group(0)}</mark>'
                # استخدم فقط إذا لم يكن النص الأصلي قد تم تمييزه بالفعل كمطابقة تامة
                # نستخدم re.sub مع count=0 لاستبدال الكل
                text = part_pattern.sub(part_replacer, text)
    return text
