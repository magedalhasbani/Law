...
def highlight_keywords(text, keywords, normalized_keywords=None, exact_match=False):
    """تلوين الكلمات المطابقة (تام وجزئي) باللون الأصفر"""
    # دائماً لوّن الكلمات المطابقة جزئياً بمارك عادي
    # ولوّن المطابقة التامة بنفس المارك لكن بخلفية أغمق للتفريق، أو العكس
    if not keywords:
        return text
    # اجعل النص غير معدل حتى لا تتداخل العلامات
    highlighted_text = text
    # أولاً لوّن التطابق التام (لو تم اختياره)
    if exact_match and normalized_keywords:
        for i, orig_kw in enumerate(keywords):
            if not orig_kw:
                continue
            norm_kw = normalized_keywords[i]
            # التطابق التام (كلمة مستقلة)
            pattern = re.compile(r"(?<!\w)" + re.escape(orig_kw) + r"(?!\w)", re.IGNORECASE)
            highlighted_text = pattern.sub(
                r'<mark class="mark-exact">\g<0></mark>', highlighted_text)
    # دائماً لون التطابق الجزئي (بدون شرط التطابق التام)
    for kw in keywords:
        if not kw:
            continue
        # إذا سبق وتم تلوينها كتطابق تام، لا تعد تلوينها
        # لون كل ظهور للكلمة حتى الجزئي
        pattern = re.compile(re.escape(kw), re.IGNORECASE)
        # لا تلوّن إذا بداخل مارك سابق
        highlighted_text = re.sub(
            r'(?<!<mark[^>]*?>)(' + re.escape(kw) + r')(?![^<]*?</mark>)',
            r'<mark class="mark-partial">\1</mark>',
            highlighted_text,
            flags=re.IGNORECASE
        )
...
def render_law_file_viewer(files):
    ...
    if law_sel:
        ...
        for para in doc.paragraphs:
            ...
            if txt:
                law_text += txt + "\n\n"
        # --- منع التحديد والنسخ في مربع القانون الكامل ---
        st.markdown("""
        <style>
        textarea[disabled], .stTextArea textarea[disabled], textarea[readonly], .stTextArea textarea[readonly] {
